	...
	for i in range (z_step):
		...
		for j in range (l_tripleprime_step):
			...
			for k in range (phi_step):
				...
				shear += factor * np.cos(2*l_phi - 2*phi_mid)  * total_halo_dust_bispectrum(zmid,kTriangle(l_mag/D_mid,l_tripleprime_mid/D_mid,l_phi - phi_mid))[0,0] * 1/(2*np.pi)**2 * delta_z * delta_phi * l_tripleprime_mid * delta_l_tripleprime
				print(i,j,k,shear)
	return (shear)